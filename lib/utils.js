...
exports.imageExists = function (filePath, log = console.log) {
    if (!fs.existsSync(filePath)) {
        ...
        log('[Device] Placeholder black image created:', filePath);
    } else {
        fs.chmodSync(filePath, 0o777);
        log('[Device] Image already exists:', filePath);
    }
};
...
exports.normalizeRemoteLines = function (text) {
    return text
        .split('\n')
        .map((line) => line.trim())
...
exports.fetchRemoteText = async function (fileName) {
    const REMOTE_TEXT_BASE_URL = 'https://raw.githubusercontent.com/martijnpoppen/com.eufylife.security/main';
    const url = `${REMOTE_TEXT_BASE_URL}/${fileName}`;

...
class EufyP2P {
    ...
    async getActorID(station_sn) {
        const hub = this.hubs[station_sn];

        Homey.app.log(`[P2P] - ${station_sn} - getActorID...`, hub);
    ...
    sendCommand(ctx, commandTypeString, station_sn, commandType, value, channel = null, valueSub = 0, strValue = '', nested = null) {
        return new Promise(async (resolve, reject) => {
            ...
            const lastUpdate = this.hubs[station_sn] && this.hubs[station_sn].lastUpdate;
            ...
            if(Homey.app.P2P[station_sn] && Homey.app.P2P[station_sn].isConnected()) {
                ...
                if(channel === null && !nested) {
                    ...
                    Homey.app.P2P[station_sn].sendCommandWithStringPayload(nested, JSON.stringify(value), channel);
                } else {

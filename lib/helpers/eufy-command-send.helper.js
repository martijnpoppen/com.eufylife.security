...
exports.sendCommand = function(commandTypeString, station_sn, commandType, value, channel = null, valueSub = 0, strValue = '', nested = false) {
    return new Promise(async (resolve, reject) => {
        ...
        if(!_devClientService[station_sn] || (_devClientService[station_sn] && !_devClientService[station_sn].isConnected())) {
        ...
        if(_devClientService[station_sn] && _devClientService[station_sn].isConnected()) {
            ...
            setTimeout(() => {
                if(channel === null && !nested) {
                    ...
                    Homey.app.log(`${station_sn} - Sending Nested Command - CMD_SET_PAYLOAD/${CommandType.CMD_SET_PAYLOAD} - ${commandTypeString}/${commandType} | value: ${value}`);
                    _devClientService[station_sn].sendCommandWithStringPayload(CommandType.CMD_SET_PAYLOAD, JSON.stringify({
                        "account_id": _settings.HUBS[station_sn].ACTOR_ID,
                        "cmd": CommandType[commandTypeString],
                        "mValue3": 0,
                        "payload": {
                            "mode_type": value,
                            "user_name": "Homey"

...
module.exports = class eufyEventsHelper {
    ...
    async updateDeviceProperties(homeyDevice, eufyDevice, name, value) {
        if (homeyDevice) {
            ...
            if (name === PropertyName.DeviceVideoWDR && homeyDevice.hasCapability('CMD_BAT_DOORBELL_WDR_SWITCH')) {
                homeyDevice.setParamStatus('CMD_BAT_DOORBELL_WDR_SWITCH', value);
            }

            if (name === PropertyName.setVideoStreamingQuality && homeyDevice.hasCapability('CMD_BAT_DOORBELL_VIDEO_QUALITY')) {
                homeyDevice.setParamStatus('CMD_BAT_DOORBELL_VIDEO_QUALITY', value);
            }

            if (name === PropertyName.DeviceAutoNightvision && homeyDevice.hasCapability('CMD_IRCUT_SWITCH')) {
                homeyDevice.setParamStatus('CMD_IRCUT_SWITCH', value);
            }

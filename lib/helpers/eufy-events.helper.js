...
module.exports = class eufyEventsHelper {
    ...
    async enforceSettings(ctx, hasBattery = false) {
        ctx.deviceList.forEach(async (d) => {
            try {
                if (d._started && ctx.homey.app.eufyClient && d.EufyDevice.hasBattery() === hasBattery) {
                    await d.deviceParams(d, true);
                } else {
                    ctx.homey.app.debug(`enforceSettings - device started? `, d._started, d.EufyDevice.hasBattery() === hasBattery);
                }
            } catch (error) {

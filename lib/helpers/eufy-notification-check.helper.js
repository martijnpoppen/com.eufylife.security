...
module.exports = class eufyNotificationCheckHelper {
    ...
    async handleDeviceNotification(device, state, type) {
        ...
        const deviceUser = device.isCamera() ? device.getPropertyValue(PropertyName.DevicePersonName) : 'Unkown person'; 
        const user = type === 'NTFY_FACE_DETECTION' ? deviceUser : null;
    ...
    async setDeviceImage(device) {
        if(device._image) {
            this.homey.app.log(`[NTFY] - 2b. update Camera Image url`);
    ...
    async triggerFlow(device, device_sn, message, state, user = null) {
        ...
        if (message.includes('alarm_')) {
            if(device.hasCapabillity(message)) {
            ...
            await this.setDeviceImage(device);
            ...
            this.homey.app[`trigger_${message}`]
                ...
                .catch(this.error)
                .then(this.homey.app.log(`[NTFY] - 3. ${device.getName()} - Triggered ${message} - with state: ${state} - tokens: `, tokens));
        } else {
            this.homey.app.log(`[NTFY] - 3. ${device.getName()} doesn't allow ${message} - with state: ${state}`);
        }
    }
};
// ---------------------------------------END OF FILE----------------------------------------------------------

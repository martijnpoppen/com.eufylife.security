    ...
    try {
        ...
        _pushClient.connect((msg) => {
            ...
            if(+new Date - _msg.sent < 11000) {
                const {event_type, message, device_sn, push_count, pic_url, value, notification_style, user} = getData(_msg);
                Homey.app.log(`[NTFY] - 1. Got ${message} - ${device_sn} - ${push_count} - ${event_type} - ${value}`, {event_type, message, device_sn, push_count, pic_url, value, notification_style});

                if(event_type && message && device_sn && (notification_style === NotificationType.MOST_EFFICIENT || notification_style === NotificationType.INCLUDE_THUMBNAIL && push_count === 1 || notification_style === NotificationType.FULL_EFFECT && push_count === 2 || !push_count || DEVICE_TYPES.FLOODLIGHT_CAMERA.some(d => device_sn.startsWith(d)))) {
                    deviceMatch(device_sn, message, value, pic_url, user)
                } else {
        ...
        Homey.app.log(`Error at registering pushservice`, e)
    }
}
...
function getData(msg) {
    ...
    const user = utils.get(msg, 'payload.payload.user_name', null);
    ...
    return {event_type, message, device_sn, push_count, pic_url, value, notification_style, user}
}

async function setDeviceImage(device, device_sn, pic_url) {
    if(pic_url && !DEVICE_TYPES.MOTION_SENSOR.some(d => device_sn.startsWith(d)) && !DEVICE_TYPES.DOOR_SENSOR.some(d => device_sn.startsWith(d))) {
...
async function deviceMatch(device_sn, message, value, pic_url, user) {
    ...
    pairedDevices.every(async device => {
        ...
        if(data.device_sn === device_sn) {
            triggerNTFY(device, device_sn, message, value, pic_url, user);
            return false;
        }
...
async function triggerNTFY(device, device_sn, message, value, pic_url, user) {
    if(DEVICE_TYPES.DOOR_SENSOR.some(d => device_sn.startsWith(d))) {
    ...
    } else if(message.startsWith('alarm_generic') && device.hasCapability('alarm_generic')) {
        const sensorValue = message === 'alarm_generic_on' ? true : false;
        await device.setCapabilityValue('alarm_generic', sensorValue);

        Homey.app.log(`[NTFY] - 3. Triggered ${message} - value: ${sensorValue}`);
    } else {
        await setDeviceImage(device, device_sn, pic_url);
        ...
        if(!message.includes('alarm_')) {
            ...
            const tokens = message === 'CMD_SET_ARMING' ? { user: user } : {}

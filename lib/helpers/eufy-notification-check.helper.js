...
module.exports = class eufyNotificationCheckHelper {
    ...
    async init() {
        try {
            ...
            this.homey.app.eufyClient.on('device someone loitering', (device, state, person) => this.handleDeviceNotification(device, state, 'NTFY_LOITERING_DETECTION'));
    ...
    async handleDeviceNotification(device, state, type, user = 'N/A') {
        const pairedDevices = this.deviceList;

...
module.exports = class eufyNotificationCheckHelper {
    ...
    async handleStream(videoStream, device, metadata) {
        try {
            ...
            const url = homeyDevice._snapshot_url ? homeyDevice._snapshot_url : Homey.env.SNAPSHOT_URL;


            streamToBuffer(videoStream).then(async (buffer) => {
                const formData = new FormData();
                formData.append('video', buffer);
                const response = await fetch(`${url}${deviceSN}`, {
                    method: 'POST',
                    body: formData,
                    headers: {
                        'x-secret': homeyDevice._snapshot_url ? '1234567890' : Homey.env.SNAPSHOT_SECRET,
                    }
                });
                ...
                this.homey.app.log(`[handleStream] - 5. Updating image snapshot: ${deviceSN}`);

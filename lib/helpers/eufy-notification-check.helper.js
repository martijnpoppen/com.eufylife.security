...
async function eufyPushClient() {
    ...
    _pushClient.connect((msg) => {
        ...
        if(+new Date - _msg.sent < 11000) {
            const {event_type, message, device_sn, push_count, pic_url, value} = getData(_msg);
            ...
            if(event_type && message && (pic_url && push_count !== 2 || push_count !== 2) && device_sn) {
                ...
                _devices.forEach(async device => {
                    ...
                    Homey.app.log(`[NTFY] - 2. Matching device_sn/station_Sn: ${data.device_sn}/${data.station_sn} - ${device_sn}`);
                    
                    if(data.device_sn === device_sn || data.station_sn === device_sn) {
...
function getData(msg) {
    ...
    return {event_type, message, device_sn, push_count, pic_url, value}
}

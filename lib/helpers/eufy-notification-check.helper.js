...
async function eufyPushClient() {
    ...
    _pushClient.connect((msg) => {
        ...
        if(+new Date - _msg.sent < 11000) {
            ...
            if(event_type && message && device_sn && (notification_style === NotificationType.MOST_EFFICIENT || notification_style === NotificationType.INCLUDE_THUMBNAIL && pic_url && push_count === 1 || notification_style === NotificationType.FULL_EFFECT && pic_url && push_count === 2 || DEVICE_TYPES.FLOODLIGHT_CAMERA.some(d => device_sn.startsWith(d)))) {

...
class FfmpegManager {
    ...
    trackSnapshotTask(deviceSn, snapshotPromise) {
        this.snapshotTasks.set(deviceSn, { promise: snapshotPromise, startedAt: Date.now() });
    ...
    async waitForSnapshot(deviceSn, timeoutMs = 10000, since = 0) {
        ...
        while (true) {
            ...
            if (entry && entry.startedAt >= since) {
                const remainingMs = timeoutMs - (Date.now() - start);
                if (remainingMs <= 0) {
                    throw new Error(`[takeSnapshot] - Timed out waiting for snapshot for device ${deviceSn}`);
                }

                return new Promise((resolve, reject) => {
                    const timer = setTimeout(() => {
                        reject(new Error(`[takeSnapshot] - Timed out waiting for snapshot for device ${deviceSn}`));
                    }, remainingMs);

...
class FfmpegManager {
    ...
    startStream(deviceSn) {
        ...
        return new Promise((resolve, reject) => {
            const ffmpegInstance = ffmpeg(this.currentVideoStream);
    ...
    async waitForSnapshot(deviceSn, timeoutMs = 10000, since = 0) {
        const status = await waitUntil(
            ...
            `Waiting for snapshot (deviceSN: ${deviceSn})`,
            300,
            timeoutMs
        );
    ...
    getFFMPEGPath() {
        ...
        if (os.arch() === 'arm64') {
            ...
            this.homey.app.log('[getFFMPEGPath] - Unsupported Homey model:', os.arch());
            throw new Error('Unsupported Homey model');
        }

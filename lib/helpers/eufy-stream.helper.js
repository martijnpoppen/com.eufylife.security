...
class FfmpegManager {
    ...
    streamHandler(videoStream, audioStream, device) {
        this.stopStream();

        const deviceSn = device.getSerial();
        const streamLength = this.homey.app.eufyClient.getCameraMaxLivestreamDuration();

        this.homey.app.log(`[handleStream] - Got handleStream: ${device.getName()} - ${deviceSn}`, { streamLength });
    ...
    takeSnapshot(deviceSn) {
        ...
        this.homey.app.log('[handleStream] - Saving snapshot to:', snapshotPath);
        ...
        return new Promise((resolve, reject) => {
            const cmd = ffmpeg(this.currentVideoStream)
                ...
                .on('end', () => {
                    ...
                    this.homey.app.log(`[handleStream] - Snapshot saved: ${snapshotPath}`);
                    if (this.proc === cmd) this.proc = null;
                ...
                .on('error', (err) => {
                    ...
                    this.homey.app.error('[handleStream] - FFmpeg error:', err);
                    if (this.proc === cmd) this.proc = null;
                    // reject(err);
                });
    ...
    stopStream(device = null) {
        try {
            if (this.proc) {
                this.homey.app.log('[FfmpegManager] Stopping proc...', device ? `for device ${device.getSerial()}` : '');
                this.proc.kill();
                this.proc = null;
            } else {
                this.homey.app.log('[FfmpegManager] No active proc to kill.');
            }

            if (this.currentVideoStream) {
                this.homey.app.log('[FfmpegManager] Stopping videostream...', device ? `for device ${device.getSerial()}` : '');
                ...
                this.homey.app.log('[FfmpegManager] No active videostream to destroy.');
            }
        } catch (error) {
            this.homey.app.error('[FfmpegManager] Error stopping stream:', error);
        }
    }
    ...
    getFFMPEGPath() {
        ...
        this.homey.app.log('[handleStream] - Using ffmpeg binary at:', ffmpegPath);

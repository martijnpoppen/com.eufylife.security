...
exports.init = async function (settings) {
    ...
    try {
        ...
        for (const key of keys) {
            const triggerId = `trigger_${key}`;

            new Homey.FlowCardTrigger(triggerId)
            .registerRunListener(onTriggerNtfy)
            .register()
            .getArgument('devicelist')
            .registerAutocompleteListener(utils.onDeviceListVariableAutocomplete);
        };
...
async function onTriggerNtfy(args, state) { 
    return new Promise(function(resolve, reject) {
        Homey.app.log(`[NTFY] - 3. Matching device_sn -> ${args.devicelist.device_sn} - ${state.device_sn}`);

        if(args.devicelist.device_sn === state.device_sn) {
            Homey.app.log(`[NTFY] - 4. Triggered ${message}`)
            resolve();
        } else {
            Homey.app.log(`[NTFY] - 4. Could not find an card with ${message} -> ${pushClientDeviceSn} - ${cardDeviceSn}`);
            reject();
        }
    ...
    _pushClient.connect((msg) => {
        if(+new Date - msg.payload.event_time < 9000) {
            ...
            if(event_type && message && push_count === 1 && device_sn) {
                Homey.ManagerFlow
                .getCard('trigger', `trigger_${message}`)
                .trigger({}, {device_sn: device_sn})
                .catch( this.error )
                .then(Homey.app.log(`[NTFY] - 2. Trigger valid -> ${message} - ${device_sn}`));
            }
        }
    });
